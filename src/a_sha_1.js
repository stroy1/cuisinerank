...
OAuth.setProperties(OAuth, // utility functions
{
    ...
    correctTimestampFromSrc: function correctTimestampFromSrc(parameterName) {
        ...
        var q = src.indexOf("?");
        if (q < 0) return;
        var parameters = OAuth.getParameterMap(OAuth.decodeForm(src.substring(q+1))); // CHANGED
        var t = parameters[parameterName];
        if (t === null) return;
        OAuth.correctTimestamp(t);*/
        OAuth.correntTimestamp(0);
    },
    /** Generate timestamps starting with the given value. */
    correctTimestamp: function correctTimestamp(timestamp) {
        //OAuth.timeCorrectionMsec = (timestamp * 1000) - (new Date()).getTime(); // CHANGED
      OAuth.timeCorrectionMsec = (new Date()).getTime();
    },
...
function fetchRestaurants(coordinates) {
  ...
  var accessor = {
    token: "HcNKxSwzXUmcmg7xQlth3Y5AxIgyakzX",
    tokenSecret: "O9KeD7ATQFPrGh-PDps_Whlz8jc",
    consumerKey: "cNbVQ-9QMnJDNHe_6FkNYQ",
    consumerSecret: "VuM0Zsqszg15ujk04q0gBe_IyY0"
  };
  
  var message = {
    action: url,
    method: "GET",
    parameters:{"term":"restaurants","limit":"5","ll": coordinates.latitude + ","+ coordinates.longitude}
  };
 ...
 // var OAuth;
  ...
  OAuth.SignatureMethod.sign(message, accessor);
  url = url + '?' + OAuth.formEncode(message.paramters);
